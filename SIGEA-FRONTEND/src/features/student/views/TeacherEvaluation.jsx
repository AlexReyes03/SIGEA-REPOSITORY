...
export default function TeacherEvaluation() {
  ...
  const handleRatingChange = (moduleId, value) => {
    setModules(modules.map((module) => (module.id === moduleId ? { ...module, rating: value } : module)));
  };

  const handleCommentChange = (moduleId, value) => {
    setModules(modules.map((module) => (module.id === moduleId ? { ...module, comment: value } : module)));
  };
  ...
  const handleSubmitEvaluation = async (moduleId) => {
    ...
    if (!moduleToEvaluate) {
      showError('Error', 'No se encontró el módulo a evaluar');
      return;
    }
    ...
    if (!validation.isValid) {
      showWarn('Datos incompletos', validation.errors[0]);
      return;
    }
    ...
    try {
      ...
      showSuccess('Evaluación enviada', 'Tu evaluación ha sido registrada exitosamente');
    } catch (error) {
      console.error('Error submitting evaluation:', error);
      showError('Error al enviar', error.message || 'No se pudo enviar la evaluación');
    }
  };
  ...
  const renderModuleCard = (module) => {
          ...
          <div>
            ...
            <div className="mb-3">
              <div className="text-center mb-2">
                <Rating value={module.rating} onChange={(e) => handleRatingChange(module.id, e.value)} cancel={false} className="custom-rating" />
              </div>
              <small className="form-label fw-medium text-start d-block">Califica el desempeño del docente</small>
            </div>

            {/* Comentario obligatorio */}
            <div className="mb-3">
              <label className="form-label fw-medium small text-muted mb-2">Comentario sobre el docente *</label>
              <div className="overflow-y-auto" style={{ maxHeight: '15rem' }}>
                <InputTextarea value={module.comment} onChange={(e) => handleCommentChange(module.id, e.target.value)} rows={2} placeholder="Comparte tu experiencia..." className="w-100" />
              </div>
              <hr />
            </div>
    ...
    return (
      <div key={module.id} className="col-12 col-sm-6 col-lg-4 col-xl-3 mb-3">
        <div className={`card h-100 border-0 ${module.isEvaluated ? 'bg-light' : ''}`} style={{ fontSize: '0.9rem', maxHeight: '37.5rem' }}>
          <div>{cardHeader}</div>
          <div className="card-body pt-0">{cardContent}</div>
        </div>
      </div>
    );
  };
    ...
    if (error) {
      return (
        <div className="col-12">
          <Message severity="error" text={error} className="w-100" />
        </div>
      );
    }
      ...
      return (
        <div className="row">
          {pendingModules.length > 0 ? (
            ...
            <div className="col-12 text-center py-5">
              <i className="pi pi-check-circle text-success" style={{ fontSize: '3rem' }} />
      ...
      return (
        <div className="row">
          {evaluatedModules.length > 0 ? (
            ...
            <div className="col-12 text-center py-5">
              <i className="pi pi-info-circle text-muted" style={{ fontSize: '3rem' }} />
      ...
      <div className="container-fluid py-5">
        <div className="row justify-content-center">
          <div className="col-md-6">
            <Message severity="warn" text="Solo los estudiantes pueden acceder a las evaluaciones docentes" className="w-100" />
          </div>
        </div>
      </div>
    );
