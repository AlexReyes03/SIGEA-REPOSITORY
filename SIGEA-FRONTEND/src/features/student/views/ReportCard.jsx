...
const BoletaPDF = ({ studentData, groupData, moduleData, campusData, startDate, endDate }) => {
  ...
  const moduleGrades = (moduleData.subjects || []).map((subject) => subject.grade).filter((grade) => grade != null && grade >= 6 && grade <= 10);

  //Calcula el promedio por modulo
  const moduleAverage = moduleGrades.length > 0 ? (moduleGrades.reduce((a, b) => a + b, 0) / moduleGrades.length).toFixed(1) : 'S/C';
        ...
        <View style={styles.header}>
          <View style={styles.headerText}>
            ...
            <Text style={styles.campusAddress}>
              {(campusData.address || '').toUpperCase()} TELÉFONO: {(campusData.phone || '').toUpperCase()}
            </Text>
            <Text style={styles.campusContact}>R.F.C. {(campusData.rfc || '').toUpperCase()}</Text>
          </View>

          <View style={styles.logoCircle}>{ImagenCetec && <Image style={styles.logoImage} src={ImagenCetec} />}</View>
        </View>
        <View style={styles.content}>
          <Text style={styles.documentTitle}>Boleta Académica</Text>
          <View style={styles.studentSection}>
            <View style={styles.studentInfo}>
              <Text style={styles.studentName}>{[studentData.name, studentData.paternalSurname, studentData.maternalSurname].filter(Boolean).join(' ').toUpperCase()}</Text>
            ...
            <View style={styles.subjectsTable}>
              ...
              {moduleData.subjects.map((subject, index) => (
                <View key={subject.subjectId || index} style={[styles.tableRow, index % 2 === 1 && styles.tableRowEven]}>
                  ...
                  <Text style={[styles.tableCellGrade, subject.grade != null ? styles.gradeText : styles.noGradeText]}>{subject.grade != null ? subject.grade : 'S/C'}</Text>
                </View>
              ))}
...
export const ReportCard = ({ studentData, groupData, moduleData, campusData, startDate, endDate }) => {
  ...
  const handleDownload = async () => {
    try {
      const blob = await pdf(<BoletaPDF studentData={studentData} groupData={groupData} moduleData={moduleData} campusData={campusData} startDate={startDate} endDate={endDate} />).toBlob();
