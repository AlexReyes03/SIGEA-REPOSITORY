...
export default function AssignStudentsPickList({ group }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            setLoading(true);
            try {
                const [allUsersRes, groupStudentsRes, studentsWithGroupRes] = await Promise.all([
                    getAllUsers(),
                    getGroupStudents(group.groupId),
                    getStudentsWithGroup()
                ]);

                const students = Array.isArray(allUsersRes) 
                    ? allUsersRes.filter(user => user.roleName === "STUDENT")
                    : [];
        ...
        if (selectedMode.code === 'ADD') {
            ...
            const studentsWithoutGroup = allStudents
                .filter(student => 
                    ...
                    !allStudentsWithGroupIds.includes(student.id)
                )
                .map(student => normalizeStudent(student));

            setSource(studentsWithoutGroup);
            setTarget([]);

        } else if (selectedMode.code === 'MOVE') {
            // Modo MOVE: Solo estudiantes que SÍ tienen grupo pero NO están en el grupo actual
            const studentsInOtherGroups = studentsWithGroup
                .filter(student => student.groupId !== group.groupId)
                .map(student => normalizeStudent(student, true));
            
            setSource(studentsInOtherGroups);
            setTarget([]);

        } else if (selectedMode.code === 'REMOVE') {
            // Modo REMOVE: Estudiantes que están en el grupo actual
            const studentsInCurrentGroup = currentGroupStudents
                .map(student => normalizeStudent(student, true));
            
            setSource(studentsInCurrentGroup);
            setTarget([]);
        }
            ...
            <div className="flex flex-wrap p-2 align-items-center gap-3">
                <div className="flex-1 flex flex-column gap-1">
                    ...
                    <div className="flex gap-3 text-500 text-sm">
                        ...
                        {item.groupId && (selectedMode?.code === 'MOVE' || selectedMode?.code === 'REMOVE') && (
                            <span className={`font-medium ${selectedMode?.code === 'REMOVE' ? 'text-red-500' : 'text-orange-500'}`}>
                                Grupo ID: {item.groupId}
                            </span>
                        )}
                    </div>
                </div>
        ...
        confirmAction({
            ...
            onAccept: async () => {
                try {
                    ...
                    const [groupStudentsRes, studentsWithGroupRes] = await Promise.all([
                        getGroupStudents(group.groupId),
                        getStudentsWithGroup()
                    ]);
        ...
        <div className="flex flex-wrap">
            ...
            >
                <span className="d-none d-sm-inline ms-2">
                    {selectedMode?.code === 'REMOVE' ? 'Quitar del grupo' : 'Guardar cambios'}
                </span>
            </Button>
        </div>
    );
    ...
    const getTargetHeader = () => {
        const actionText = selectedMode?.code === 'ADD' ? 'añadir' : 
                          selectedMode?.code === 'MOVE' ? 'mover' : 'quitar';
        
        const severity = selectedMode?.code === 'REMOVE' ? 'danger' : 
                        selectedMode?.code === 'MOVE' ? 'info' : 'success';
    ...
    return (
        <div className="card border-0">
            <Toolbar className="py-2 border-0 border-bottom" end={toolbarRight} start={toolbarLeft} />
            ...
            {selectedMode?.code === 'REMOVE' && sourceCount > 0 && (
                <div className="mt-3">
                    <Message 
                        severity="warn" 
                        text="Los estudiantes eliminados del grupo quedarán sin grupo asignado hasta que sean añadidos a otro grupo." 
                    />
                </div>
            )}

            {/* Panel de estadísticas con Tags */}
            <div className="mt-3 p-3 bg-gray-50 border-round">
                <div className="flex flex-wrap gap-3 justify-content-center">
                    <div className="flex align-items-center gap-2">
                        <span className="text-600 font-medium">Total estudiantes: </span>
                        <Tag icon="pi pi-users" value={allStudents.length} severity="info" />
                    </div>
                    <div className="flex align-items-center gap-2">
                        <span className="text-600 font-medium">En este grupo: </span>
                        <Tag icon="pi pi-check-circle" value={currentGroupStudents.length} severity="success" />
                    </div>
                    <div className="flex align-items-center gap-2">
                        <span className="text-600 font-medium">Con grupo: </span>
                        <Tag icon="pi pi-sitemap" value={studentsWithGroup.length} severity="warning" />
                    </div>
                    <div className="flex align-items-center gap-2">
                        <span className="text-600 font-medium">Sin grupo: </span>
                        <Tag icon="pi pi-user-plus" value={studentsWithoutGroup} severity="help" />
                    </div>
                </div>
            </div>
        </div>
